...
> 📌 **생성 출력 안내**
>
> 시연 과정에서 입력한 프롬프트 문장이 화면에 보이지 않았던 이유는, 기존 프로그램이 **프롬프트와 300자의 생성 결과를 한 번에 출력**하면서 터미널 자동 스크롤로 첫 줄이 화면 위로 밀렸기 때문입니다.
>
> 프롬프트가 누락된 것은 아니며, 생성 결과의 맨 앞에 항상 그대로 포함됩니다. 확률적으로 생성되는 부분은 **프롬프트 뒤의 이어쓰기**뿐입니다. 현재는 두 부분을 다음과 같이 구분해 출력합니다.
>
> ```text
> === 입력 프롬프트: 모델 생성 부분이 아님 ===
> 인공지능은 금융 시장을 이해하는 새로운 도구가 될 수 있다.
>
> === 모델이 새로 생성한 이어쓰기: 300자 ===
> <모델이 생성한 이어쓰기>
> ```
